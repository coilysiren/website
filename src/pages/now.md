---
title: "Now"
---

> A [/now page](https://nownownow.com/about) — what I'm focused on at this point in my life, refreshed periodically. Last updated: April 2026.

## Building

I'm in an aggressive build phase. The thing I'm most excited about right now is **[Gauntlet](https://github.com/coilyco-flight-deck/gauntlet)** — a two-agent adversarial loop that infers software correctness by watching how code behaves under sustained, targeted attack in a [dark factory](https://en.wikipedia.org/wiki/Lights_out_(manufacturing)) environment. No pressure to ship fast; the goal is to make the thing real, then write it up here.

Day job is platform/SRE work that's slowly bending toward AI agents and observability — going deep on state-of-the-art agent and MCP patterns.

In parallel, I'm modernizing this very website. Part of building this page involved building the script that pulls in all the data that fed the writing of it — the page is, in a real sense, generating itself. I'm also maintaining a small herd of [Eco game mods](https://github.com/coilysiren?tab=repositories&q=eco), tinkering with my [Bluesky apps](/apps/bsky-popularity-contest/), and running a personal CRM project out of an Obsidian vault. Across the past month I've been active across roughly 35 repos. The throughline of my whole career has been *building things that give engineering teams more leverage*. Now I'm pointing the same instinct at my own work, with AI as the accelerant.

I just upgraded to **Claude Max** and I'm pushing it pretty hard. Setting up remote dispatch on my desktop so I can run agents from anywhere. The amount of functional IP I can personally produce in a week has changed dramatically in the last month, and I'm still figuring out what that means.

## Community

Most of my online community lives on **Bluesky** these days. I've been finding it more amenable than Reddit lately, even with the heavy anti-AI sentiment in some corners. I recently did a full social graph audit and pruned hard — I'd rather track ~100 people I actually have relationships with than scroll a firehose of strangers. The community I see in that map is overwhelmingly queer trans women in tech, with meaningful clusters around game dev, observability, Portland, and Seattle.

I'm also active in **Out in Tech** and a queer parents group, and I'm doing some childcare mutual aid in that orbit. I'm always looking for tighter-knit, more in-person community in the Bay Area.

## Reading & Watching

I'm in catch-up mode on AI/tech news, working through a backlog and trying to settle into a sustainable info diet. Currently chewing on OpenAI's *Industrial Policy for the Intelligence Age* paper. Watching a lot of Sam Altman / a16z / agent-engineering content alongside Pod Save, Veritasium, SciShow, NASA, and KPIX (Bay Area civics).

After moving left for a long stretch, I'm finding myself drifting back toward an [accelerationist](https://en.wikipedia.org/wiki/Accelerationism) position now that AI has actually arrived. Wrestling with what that means, in public, mostly on Bluesky.

## Gaming

Big chunks of my decompression time lately have gone into **[AETHUS](https://store.steampowered.com/app/2939320/AETHUS/)** — about 60 hours in the last two weeks. I'm also gearing up for a fresh **[Eco](https://en.wikipedia.org/wiki/Eco_(2018_video_game))** cycle starting at the end of this week, and **[Wildermyth](https://en.wikipedia.org/wiki/Wildermyth)** is in heavy rotation.

## Life

I live in **[the East Bay](https://en.wikipedia.org/wiki/East_Bay_(San_Francisco_Bay_Area))**. I'm in the middle of planning an **[ADU](https://en.wikipedia.org/wiki/Secondary_suite)** for my house — picked a preliminary design recently, very excited about it. Recovering from a surgery, which has temporarily grounded me from biking (and my bike is broken anyway, perfect timing).

Currently building a few daily habits: **Duolingo** (about a month deep now), and **leetcode** (deferred a week — too much else going on right now to add a third habit responsibly). Also recently lost some weight and hit a historical low, which is its own thing.

---

#### Footnotes — obscure things on this page

In case any of these jumped out at you and you want to learn more:

- **dark factory** — https://en.wikipedia.org/wiki/Lights_out_(manufacturing)
- **accelerationism** — https://en.wikipedia.org/wiki/Accelerationism
- **Eco (the game)** — https://en.wikipedia.org/wiki/Eco_(2018_video_game)
- **Wildermyth** — https://en.wikipedia.org/wiki/Wildermyth
- **East Bay** — https://en.wikipedia.org/wiki/East_Bay_(San_Francisco_Bay_Area)
- **ADU (Accessory Dwelling Unit)** — https://en.wikipedia.org/wiki/Secondary_suite

---

*Inspired by [Derek Sivers' /now page movement](https://nownownow.com/about). Generated semi-collaboratively with Claude from my GitHub, Bluesky, YouTube, Reddit, and Steam activity — see [the source on GitHub](https://github.com/coilysiren/website).*
