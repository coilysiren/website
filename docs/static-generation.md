...
live in the shared base layout, along with a default `og:image` a page can
override per route. See [social card](social-card.md). Per-page image
generators stay intentionally absent: the card is one committed asset, not a
build step.

`feed.xml` is an Atom feed carrying exactly the promoted posts, newest first,
generated by [`src/feed.njk`](../src/feed.njk) from the same `promoted` flag
`/writing/` filters on. It uses the existing date filters rather than
`@11ty/eleventy-plugin-rss`, so the feed adds no dependency.
